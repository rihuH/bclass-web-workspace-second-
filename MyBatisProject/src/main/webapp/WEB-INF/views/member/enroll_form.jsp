		...
		function idCheck(){
			...
			$.ajax({
				...
				success : function(response){
					...
					if(response === "NNNNN"){
						$('#id-result').css('fond-size', '12px').css('color','red').text('이미 존재하는 아이디입니다.')
					} else{
						$('#id-result').css('font-size','12px').css('color','green').text('사용가능한 아이디 입니다')
					}
				},
			});
			} else{
				$('#id-result').css('fond-size', '12px').css('color','red').text('사용 불가능한 아이디입니다.')
			}
		}
			...
			<table
				...
				<tr>
					<td><input type="text" name="userId" id="user_id"
						...
						placeholder="숫자와 영어로 4-10자" onkeyup="idCheck()"> <!-- ************ajax*************** -->
						</td>
